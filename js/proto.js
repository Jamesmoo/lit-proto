...
const dropdowns = {
  columnOptions: [
    ...
    {
      value: "Alternate Management Assistant",
      ddValue: "alternateManagementAssistant"
    }
  ],
  operatorOptions: [
  ...
  primarySupervisorOptions: uniqueObjBy(
    orgs
      .map(org => {
        return new KeyValDD(
          org.PRIMARY_SUPERVISOR_NAME,
          org.PRIMARY_SUPERVISOR_ID
        );
      })
      .sort(compareByProperty("value")),
    x => x.ddValue
  ),
  altSupervisorOptions: uniqueObjBy(
    orgs
      .map(org => {
        return new KeyValDD(
          org.ALTERNATE_SUPERVISOR_NAME,
          org.ALTERNATE_SUPERVISOR_ID
        );
      })
      .sort(compareByProperty("value")),
    x => x.ddValue
  ...
  timeKeeperOptions: uniqueObjBy(
    orgs
      .map(org => {
        return new KeyValDD(
          org.PRIMARY_TIMEKEEPER_NAME,
          org.PRIMARY_TIMEKEEPER_ID
        );
      })
      .sort(compareByProperty("value")),
    x => x.ddValue
  ),
  altTimeKeeperOptions: uniqueObjBy(
    orgs
      .map(org => {
        return new KeyValDD(
          org.ALTERNATE_TIMEKEEPER_NAME,
          org.ALTERNATE_TIMEKEEPER_ID
        );
      })
      .sort(compareByProperty("value")),
    x => x.ddValue
...
const dropdownOptions = options => html`
  ${options.map(
    opt =>
      html`
        <option value="${opt.ddValue}">${opt.value}</option>
      `
  )}
`;
...
const searchBox = dropdowns => html`
  ...
  Search active organizations only ​
